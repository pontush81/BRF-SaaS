...
// Set a consistent seed for faker to get reproducible results
faker.seed(123);
    ...
    for (let i = 0; i < unitCount; i++) {
      await prisma.unit.create({
        data: {
          ...
          size: faker.number.float({ min: 35, max: 120, fractionDigits: 1 }),
    ...
    await prisma.issue.create({
      data: {
        ...
        status: issueStatuses[
          Math.floor(Math.random() * issueStatuses.length)
        ] as any,
        priority: issuePriorities[
          Math.floor(Math.random() * issuePriorities.length)
        ] as any,
        organizationId: organization.id,
      },
    });
