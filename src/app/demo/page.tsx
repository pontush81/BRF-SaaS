...
export default function DemoPage() {
  ...
  const [activePage, setActivePage] = useState(DEMO_SECTIONS[0].pages[0]);
  const [showMobileMenu, setShowMobileMenu] = useState(false);
  
  const handleSectionClick = (section: typeof DEMO_SECTIONS[0]) => {
    setActiveSection(section);
    setActivePage(section.pages[0]);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="bg-gradient-to-r from-blue-600 to-blue-700 py-4 px-4 text-white sticky top-0 z-10">
        <div className="container mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="text-sm bg-yellow-500 text-blue-900 px-2 py-1 rounded-md font-medium">DEMO</div>
          </div>
                    ...
                    {section.pages.map((page) => (
                      <li key={page.id}>
                        <button
                          ...
                          className={`w-full text-left px-3 py-1.5 rounded-md ${
                            activeSection.id === section.id && activePage.id === page.id
                              ? 'bg-blue-100 text-blue-700 font-medium'
                              : 'text-gray-600 hover:bg-blue-50'
                          }`}
                        >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-blue-900 text-white p-4 flex justify-between items-center">
        <p className="text-sm md:text-base">
          Detta är en demonstration av den digitala handboken för bostadsrättsföreningar.
        </p>
        <Link 
          href="/register" 
