...
export async function POST(request: NextRequest) {
  try {
    ...
    // Parse request body
    const { organizationId, planType, successUrl, cancelUrl } =
      await request.json();
    ...
    if (!isProd) {
      console.log(
        `[Stripe Checkout] Creating session for org ${organizationId}, plan ${planType}`
      );
    }
    ...
    const finalSuccessUrl =
      successUrl || `${baseUrl}/subscription/success?org=${organizationId}`;
    const finalCancelUrl =
      cancelUrl || `${baseUrl}/subscription?org=${organizationId}`;
