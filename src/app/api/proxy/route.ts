...
console.log(
  '[Proxy] Supabase URL:',
  SUPABASE_URL ? `${SUPABASE_URL.substring(0, 15)}...` : 'MISSING'
);
...
try {
  if (SUPABASE_URL && SUPABASE_URL.trim() !== '') {
    ...
    projectId = urlObj.hostname.split('.')[0] ?? 'unknown';
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'Content-Type, Authorization, X-Client-Info, apikey, X-Supabase-Proxy',
...
async function handleHealthCheck(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    try {
      ...
      const dnsCheckResponse = await fetch(
        `${SUPABASE_URL}/dns-check-${Date.now()}`,
        {
          method: 'HEAD',
          mode: 'no-cors',
          cache: 'no-store',
    ...
    try {
      ...
      if (response.ok) {
        console.log(
          `[Proxy/Health] Health check successful in ${responseTime}ms`
        );
        return NextResponse.json(
          {
            reachable: true,
        ...
        console.error(
          `[Proxy/Health] Health check failed with status ${response.status}`
        );
        return NextResponse.json(
          {
            reachable: false,
      ...
      console.error(
        `[Proxy/Health] Failed to parse response as JSON:`,
        jsonError
      );
...
async function forwardToSupabase(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.log(
      `[Proxy/Forward] Path extraction: Original: ${originalPath}, Extracted: ${supabasePath}`
    );
    ...
    console.log(
      '[Proxy/Forward] Response headers:',
    ...
    if (isAuthEndpoint) {
      try {
        ...
        console.log(
          '[Proxy/Forward] Auth response body (first 100 chars):',
        ...
        if (!responseText && supabaseResponse.ok) {
          console.warn(
            '[Proxy/Forward] Empty response body from Supabase auth endpoint'
          );
          return NextResponse.json(
...
export async function GET(request: NextRequest) {
  ...
  if (firstSegment === 'health') {
    ...
    console.log(
      `[Proxy] Forwarding request to Supabase, segments:`,
      pathSegments
    );
